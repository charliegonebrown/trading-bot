...
# ── Debug dump — shows what Railway actually injected ─────────────────────────
print("STARTUP ENV DUMP:")
...
if not DATABASE_URL:
    ...
    if all([PG_HOST, PG_USER, PG_PASSWORD]):
        ...
        print(f"  Built from PG* vars → {PG_HOST}/{PG_DB}")
    else:
        DATABASE_URL = "sqlite:///./trading_bot.db"
        print("  WARNING: falling back to SQLite")

print(f"  FINAL URL    = {DATABASE_URL[:50]}")
...
if IS_SQLITE:
    ...
    print(f"  PostgreSQL engine created → {safe_url}")
